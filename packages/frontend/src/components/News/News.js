...
const News = ({ allNews, setOpen, socket }) => {
    ...
    return (
      <div className={"NewsContainer"}>
        {
          deleteValue.flag && <DeleteNewsOrTeaching
            setOpen={setDeleteValue} open={deleteValue} value={deleteValue.value}
            socket={socket}
            component={"news"}
          />
        }
        {redNew.flag && <div style={window.innerWidth > 600 ? {width: '70%'}: {width: '100%'}} className={"redNews"}>
          <div className={"NewsWrapper2"}>
            ...
            <div className={"newsInfo2Container"}>
              <div className={"newsInfo2"}>
                {redNew.news.description}
              </div>
            </div>

            <div className={"newsButton2"}>
              <button onClick={closeNews} className={"readNewsButton"}>Закрити</button>
              {
                role === "admin" && <button onClick={deleteNews} className={"readNewsButton"}>Видалити</button>
              }
            </div>
          </div>
        </div>
        ...
        {
          !redNew.flag ?  <div style={window.innerWidth > 600 ? {height: '87%'}: {height: '77%'}} className={redNew.flag ? "NewsContainerSkrol2" : "NewsContainerSkrol"}>
            {role === "admin" && window.innerWidth > 600 && (
              <div className={"addNews"} onClick={() => {
                setOpen(true);
              }}>
          ...
          </div> : window.innerWidth  > 600 && <div className={redNew.flag ? "NewsContainerSkrol2" : "NewsContainerSkrol"}>
            {role === "admin" && window.innerWidth > 600 && (
              <div className={"addNews"} onClick={() => {
                setOpen(true);
              }}>
