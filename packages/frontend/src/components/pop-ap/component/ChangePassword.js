...
const ChangePassword = ({ changeComponent, setUpdateProfile, updateProfile, setSuccess }) => {
  ...
  const isCoincidencePassword =
    netPassword.value.toString().trim() === repeatPassword.value.toString().trim();
  ...
  return (
    <>
      {changePasswordDiv &&
        changePasswordDiv.map((value, index) => (
          <div key={index} className={'inputChangeContainer'}>
            ...
            {value.nameInput === 'repeatPassword' &&
              repeatPassword.isDirty &&
              !isCoincidencePassword &&
              !repeatPassword.flag && <div className={'inputError3'}>{t('Password_invalid2')}</div>}
          </div>
        ))}
      <button
