...
const Teaching = ({ allTeaching, setOpen, socket }) => {
  ...
  const [redTeachingIndex, setRedTeachingIndex] = useState(0);
  ...
  const redTeaching2 = (index) => {
    setRedTeachingIndex(index);
  };
  ...
  return (
    <div className={'TeachContainer'}>
      {redTeachingState.flag && (
        <div
          style={window.innerWidth > 600 ? { width: '70%' } : { width: '100%' }}
          className={'redMethodology'}
        >
          <div className={'methodologyWrapper2'}>
            <div className={'methodologyTime2'}>
              Методичка №{redTeachingState?.teaching[1][redTeachingIndex]?.lesson}
            </div>
            <div className={'methodologyTitle2'}>
              {redTeachingState?.teaching[1][redTeachingIndex].title}
            </div>
            <div className={'methodologyInfo2Container'}>
              <div className={'methodologyInfo2'}>
                <div className={'methodologyDescription1'}>
                  {redTeachingState?.teaching[1][redTeachingIndex]?.description?.images &&
                    Object.entries(
                      redTeachingState?.teaching[1][redTeachingIndex]?.description?.images
                    )
                  ...
                  {redTeachingState?.teaching[1][redTeachingIndex]?.description &&
                    redTeachingState?.teaching[1][redTeachingIndex]?.description.text.map(
                      (t, index) => (
                        <div key={index} className={'description'}>
                          ...
                          {redTeachingState?.teaching[1][redTeachingIndex]?.description?.images
                            ?.filter(
                              (image) =>
                                Object.entries(image)[0][0].split(' ')[1] === (index + 1).toString()
                            )
                            .map((i) => (
                              <div className={'imTestContainer'}>
                                <img
                                  className={'imTest'}
                                  src={Object.entries(i)[0][1]}
                                  alt={'fkfkfkr'}
                                />
                              </div>
                            ))}
            ...
            <div className={'methodologyButton2'}>
              <button onClick={closeTeaching} className={'readMethodologyButton'}>
                Закрити
              </button>
            </div>
          </div>
      ...
      {redTeachingState.flag && (
        <div className={redTeachingState.flag ? 'TeachContainerSkrol2' : 'TeachContainerSkrol'}>
          ...
          {redTeachingState.teaching[1].map((teaching, index) =>
            index !== redTeachingIndex ? (
...

      {
        window.innerWidth > 600 && !redTeachingState.flag ? (
          <div
            ...
            className={redTeachingState.flag ? 'TeachContainerSkrol2' : 'TeachContainerSkrol'}
          >
            {role === 'admin' && (
              <div
                className={'addTeaching'}
            ...
            {allTeaching.map((teaching, index) => (
              <div key={index} className={'teachingWrapper'}>
                <div className={'teachingTime'}>Предмет {teaching[0]}</div>
                <div className={'teachingTitle'}>{teaching.title}</div>
                <div className={'teachingButton'}>
                  <button
                    onClick={() => {
          ...
          !redTeachingState.flag && (
            <div
              style={
                window.innerWidth > 600
                  ? { height: '87%' }
                  : { height: '77%', justifyContent: 'center' }
              }
              className={redTeachingState.flag ? 'TeachContainerSkrol2' : 'TeachContainerSkrol'}
            >
              {role === 'admin' && (
